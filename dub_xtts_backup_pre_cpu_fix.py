...
def _init_xtts():
    ...
    try:
        ...
        # Configuração otimizada para clonagem de voz COM GPU FORÇADA
        _XTTS = TTS(model_name="tts_models/multilingual/multi-dataset/xtts_v2", gpu=True)
        
        # Mover EXPLICITAMENTE o modelo para GPU
        ...
        if hasattr(_XTTS, 'synthesizer') and hasattr(_XTTS.synthesizer, 'tts_config'):
            ...
            print("✅ [GPU] Configurações de clonagem otimizadas")
        
        # Verificar uso de memória GPU
        gpu_memory_used = torch.cuda.memory_allocated(0) / 1024**3
        print(f"📊 [GPU] Memória GPU em uso: {gpu_memory_used:.2f} GB")
        print("✅ [GPU] XTTS v2 inicializado com GPU FORÇADA")
...
def synth_tts_clone_xtts(text: str, voice_ref_wav: Path, out_wav: Path, language: str = LANG_DEFAULT):
    ...
    try:
        ...
        # 🎯 CORREÇÃO: Verificar GPU antes da síntese
        import torch
