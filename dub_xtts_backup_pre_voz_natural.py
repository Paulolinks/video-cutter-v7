...
def _init_xtts():
    ...
    try:
        ...
        if gpu_available:
            ...
            try:
                # Forçar inicialização CPU com parâmetros explícitos
                import os
                ...
                print(f"⚠️ [CPU] Erro na inicialização CPU: {cpu_error}")
                # Tentar fallback mais simples
                try:
                    _XTTS = TTS(model_name="tts_models/multilingual/multi-dataset/xtts_v2")
                    print("✅ [CPU] XTTS v2 inicializado com fallback")
                except Exception as fallback_error:
                    print(f"❌ [CPU] Erro no fallback: {fallback_error}")
                    raise RuntimeError(f"Falha ao inicializar XTTS com CPU: {fallback_error}")
...
def _time_stretch_to_duration(wav_in: Path, wav_out: Path, target_sec: float, sr=SR):
    ...
    # Usar asetrate+aresample para time-stretching mais preciso
    print(f"🎵 [TIME-STRETCH] Aplicando asetrate+aresample (ratio={ratio:.3f})")
    new_rate = int(sr / ratio)  # Inverso do ratio para asetrate
    cmd = ["ffmpeg","-y","-i",str(wav_in),
...
def synth_tts_clone_xtts(text: str, voice_ref_wav: Path, out_wav: Path, language: str = LANG_DEFAULT):
    ...
    try:
        ...
        import torch
        if torch.cuda.is_available() and hasattr(xtts, 'synthesizer'):
