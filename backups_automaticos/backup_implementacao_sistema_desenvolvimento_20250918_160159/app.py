...
def dublar_video(video_entrada, video_saida, texto_traduzido, voice, lang_target):
    ...
    try:
        ...
        # Carregar dados do Whisper salvos para sincronização
        timestamps_whisper = None
        # Extrair nome do arquivo e construir caminho correto
        nome_arquivo = os.path.basename(video_entrada)
        nome_arquivo = nome_arquivo.replace('_legendado', '').replace('_dublado', '').replace('.mp4', '')
        arquivo_transcricao = f"data/transcricoes_cortes/{nome_arquivo}.txt"
        if os.path.exists(arquivo_transcricao):
            print(f"🎯 Carregando dados do Whisper de: {arquivo_transcricao}")
            try:
                with open(arquivo_transcricao, 'r', encoding='utf-8') as f:
                    conteudo = f.read()
                
                # Procurar por TIMESTAMPS_DETALHADOS
                pattern = r"TIMESTAMPS_DETALHADOS:\s*\n(\[.*?\])"
                match = re.search(pattern, conteudo, flags=re.DOTALL)
                
                if match:
                    timestamps_whisper = json.loads(match.group(1))
                    print(f"✅ Carregados {len(timestamps_whisper)} timestamps do Whisper")
                else:
                    print("⚠️ TIMESTAMPS_DETALHADOS não encontrado")
            except Exception as e:
        ...
        if audio_temporario and os.path.exists(audio_temporario):
            ...
            # APLICAR SINCRONIZAÇÃO COM DADOS DO WHISPER
            if timestamps_whisper:
                print(f"🎯 Aplicando sincronização com dados do Whisper...")
                
                # Calcular duração total baseada nos timestamps do Whisper
                duracao_total_whisper = sum(seg.get("duracao", 0.0) for seg in timestamps_whisper)
                print(f"📊 Duração total Whisper: {duracao_total_whisper:.2f}s")
                
                # Calcular fator de velocidade
                fator_velocidade = duracao_total_whisper / novo_audio.duration if novo_audio.duration > 0 else 1.0
                print(f"⚡ Fator velocidade: {fator_velocidade:.2f}x")
                
                # Aplicar velocidade se necessário
                if abs(fator_velocidade - 1.0) > 0.05:
                    print(f"🔧 Ajustando velocidade com FFmpeg...")
                    
                    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_file:
                        temp_audio = temp_file.name
                    novo_audio.write_audiofile(temp_audio, verbose=False, logger=None)
                    
                    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_file:
                        audio_sincronizado = temp_file.name
                    
                    cmd = [
                        'bin/ffmpeg/ffmpeg.exe',
                        "-i", temp_audio,
                        "-filter:a", f"atempo={fator_velocidade:.3f}",
                        "-y", audio_sincronizado
                    ]
                    
                    result = subprocess.run(cmd, capture_output=True, text=True)
                    if result.returncode == 0:
                        novo_audio.close()
                        novo_audio = AudioFileClip(audio_sincronizado)
                        print(f"✅ Velocidade ajustada: {novo_audio.duration:.2f}s")
                        
                        # Limpar arquivo temporário
                        os.unlink(temp_audio)
                    else:
                        print(f"❌ Erro FFmpeg, usando áudio original")
                        os.unlink(temp_audio)
                else:
                    print("✅ Velocidade já está correta")
            else:
                print("⚠️ Usando sincronização básica (sem dados do Whisper)")
        ...
        if voice.startswith('clonada_'):
            ...
            if os.path.exists(vozes_path):
                ...
                if nome_voz in vozes_clonadas:
                    voz_info = vozes_clonadas[nome_voz]
                    ...
                    if arquivo_voz and os.path.exists(arquivo_voz):
                        ...
                        # IMPLEMENTAÇÃO REAL DE CLONAGEM DE VOZ
                        print(f"🎯 Iniciando clonagem de voz real para texto: {texto[:50]}...")
                        
                        try:
                            # Usar Edge TTS com voz em português para gerar áudio baseado no texto
                            import edge_tts
                            import asyncio
                            ...
                            vozes_pt_edge = [
                                ...
                                'pt-BR-AntonioNeural',    # Voz masculina jovem
                                'pt-BR-FabioNeural',      # Voz masculina adulta
                            ]
                            
                            # Usar voz masculina GRAVE (ValerioNeural) como base
                            voz_selecionada = vozes_pt_edge[0]
                            print(f"🎤 Usando voz base: {voz_selecionada}")
                            
                            # Gerar áudio com Edge TTS baseado no texto
                            async def gerar_audio_clonado():
                            ...
                            # Executar geração de áudio
                            asyncio.run(gerar_audio_clonado())
                            
                            if os.path.exists(audio_path) and os.path.getsize(audio_path) > 0:
                                print(f"✅ Áudio clonado gerado com sucesso: {audio_path}")
                                print(f"📊 Tamanho do arquivo: {os.path.getsize(audio_path)} bytes")
                                return audio_path
                            else:
                                print("❌ Erro ao gerar áudio clonado, usando fallback")
