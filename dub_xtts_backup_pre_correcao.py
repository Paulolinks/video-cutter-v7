...
def _init_xtts():
    """Inicializa XTTS v2 apenas quando necessário"""
    global _XTTS, _XTTS_IMPORT_ERROR
    ...
    try:
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("CUDA não está disponível! GPU é obrigatória para este sistema.")
        
        print(f"🚀 Inicializando XTTS v2 com GPU: {torch.cuda.get_device_name(0)}")
        ...
        # Configuração otimizada para clonagem de voz
        ...
        print("✅ XTTS v2 inicializado com sucesso na GPU")
        return _XTTS
...
def synth_tts_clone_xtts(text: str, voice_ref_wav: Path, out_wav: Path, language: str = LANG_DEFAULT):
    ...
    try:
        ...
        # Preparar voz de referência otimizada
