...
# dentro de carregar_modelo_spacy(idioma) codigo para baixar o spacy automatico porem ta dando erro

def carregar_modelo_spacy(idioma):
    from spacy.cli import download
    
    nome = "pt_core_news_sm" if idioma == "pt" else "en_core_web_sm"
    try:
        nlp = spacy.load(nome)
    except OSError:
        try:
            print(f"⚙️ Baixando modelo spaCy: {nome} ...")
            download(nome)
            nlp = spacy.load(nome)
        except Exception:
            ...
            print(f"⚠️ Usando spaCy blank('{lang_code}') (sem modelo treinado).")
    return nlp
...
def extrair_frases_de_efeito(texto, idioma, segmentos, tempo_min=15.0, tempo_max=53.0, similaridade_min=0.70):
